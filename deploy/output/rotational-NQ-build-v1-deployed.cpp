// STUDY VERSION LOG
// Base: ATEAM_ROTATION_V3_CHOP (LP-1.1)
// Fork: ATEAM_ROTATION_V3_CHOP (adds choppiness entry filter)
//
// archetype: rotational
// @study: ATEAM Rotation V3 Chop
// @version: CHOP-1.0
// @author: ATEAM
// @type: trading-system + batch-test
// @summary: Forked from LP-1.1. Adds choppiness entry filter alongside
//           existing SpeedRead filter. Reads choppiness from
//           ATEAM_ChoppinessFilter study via inter-study reference.
//           Entry blocked when choppiness >= threshold (default 0.10).
//           SpeedRead filter retained on Inputs 8-11 (unchanged).
//           Choppiness filter on Inputs 16-18.
//           Validated: P1+P2, E[R] $3 -> $55, 82% WR, 18% SR.
//
// CHANGE LOG
// ----------
// [2026-03-29 CHOP-1.0] Fork from LP-1.1
//   - Added Input 16: Enable Choppiness Filter (default ON)
//   - Added Input 17: Choppiness Study Ref (ATEAM_ChoppinessFilter, subgraph 0)
//   - Added Input 18: Choppiness Threshold (default 0.10)
//   - Added choppiness gate before entry: skip entry when chop >= threshold
//   - SpeedRead filter (Inputs 8-11) fully retained and independent
//   - Both filters can be active simultaneously
//   - No flatten on chop filter (just skip new entries while chop is high)
//   - Renamed DLL and study function to ATEAM_ROTATION_V3_CHOP
//
// PRIOR LOG (from LP-1.1):
// [2026-03-25 LP-1.1] RTH gate + CSV test mode
//
// Original: Forked from V2803 for LucidProp project. Adds CSV test mode for
...
SCDLLName("ATEAM_ROTATION_V3_CHOP")
...
{
    SCString FilePath;
    FilePath.Format("%s\\ATEAM_ROTATION_V3_CHOP_log.csv", sc.DataFilesFolder().GetChars());
    ...
    int ChopFilterOn, float ChopThresh, int ChopLookback)
{
    ...
    // ---------- [CHOP-2.0] Two-pass choppiness: pre-compute per-tick values ----------
    // Python pre-computes all agg bars then maps choppiness back to ticks.
    // Each tick sees its OWN bar's choppiness (not the previous completed bar).
    // To match: pass 1 aggregates + computes choppiness, pass 2 simulates.
    const int AGG_SIZE = 250;
    const int MAX_AGG_BARS = 5000;  // 500K ticks / 250 = 2000 bars max

    // Per-tick choppiness (NaN until warmup satisfied)
    float* tickChop = nullptr;
    if (ChopFilterOn)
    {
        tickChop = (float*)sc.AllocateMemory(nBars * sizeof(float));
        if (!tickChop)
        {
            sc.AddMessageToLog("CSV TEST MODE LP: Failed to allocate tickChop", 1);
            sc.FreeMemory(cycles); sc.FreeMemory(events); sc.FreeMemory(bars);
            return;
        }

        // --- Pass 1: aggregate ticks into bars, compute choppiness, map back ---
        ...
        if (!aggCloseArr || !aggRangeArr || !aggChopArr || !tickToAgg)
        {
            sc.AddMessageToLog("CSV TEST MODE LP: Failed to allocate agg arrays", 1);
            ...
            sc.FreeMemory(tickChop); sc.FreeMemory(cycles);
            sc.FreeMemory(events); sc.FreeMemory(bars);
            return;
        }

        // Aggregate (mirrors Python _aggregate_bars exactly)
        int   aggIdx = 0;
        int   tickCount = 0;
        int   prevDate = -1;
        float curHigh = -1e9f, curLow = 1e9f, curClose = 0.0f;

        for (int i = 0; i < nBars; i++)
        {
            int curDate = bars[i].DateInt;

            if (curDate != prevDate && prevDate >= 0)
            {
                // Session boundary: close partial bar, start new
                if (tickCount > 0)
                {
                    aggCloseArr[aggIdx] = curClose;
                    aggRangeArr[aggIdx] = curHigh - curLow;
                    aggIdx++;
                }
                tickCount = 0;
            }
            ...
            {
                if (bars[i].High > curHigh) curHigh = bars[i].High;
                if (bars[i].Low  < curLow)  curLow  = bars[i].Low;
                curClose = bars[i].Last;
            }
            tickToAgg[i] = aggIdx;
            tickCount++;
        ...
        // Compute choppiness per agg bar (matches Python _choppiness_ratio)
        for (int a = 0; a < nAgg; a++)
        {
            if (a < ChopLookback - 1)
            {
                aggChopArr[a] = -1.0f;  // sentinel for NaN
                continue;
            }
            float netMove = fabs(aggCloseArr[a] - aggCloseArr[a - ChopLookback + 1]);
            float sumRange = 0.0f;
        ...
        // Map choppiness to tick resolution
        for (int i = 0; i < nBars; i++)
        {
            int a = tickToAgg[i];
            tickChop[i] = aggChopArr[a];
        }

        {
            SCString msg;
            msg.Format("CSV TEST MODE LP: Precomputed choppiness — %d agg bars, %d ticks", nAgg, nBars);
            sc.AddMessageToLog(msg, 0);
        }
    ...
    for (int i = 0; i < nBars; i++)
    {
        ...
        if (posQty == 0 && anchorPrice == 0.0)
        {
            ...
            // [CHOP-2.0] Choppiness gate: skip seed if chop >= threshold
            if (ChopFilterOn && tickChop && tickChop[i] >= 0.0f && tickChop[i] >= ChopThresh)
                continue;
        ...
        if (inFavor)
        {
            ...
            // [CHOP-2.0] Choppiness gate: skip reversal re-entry if chop >= threshold
            if (ChopFilterOn && tickChop && tickChop[i] >= 0.0f && tickChop[i] >= ChopThresh)
            {
                ResetState();
                StartNewWatch(i);
...
//  Main study function
// =========================================================================
SCSFExport scsf_ATEAM_ROTATION_V3_CHOP(SCStudyInterfaceRef sc)
{
    SCSubgraphRef sg_FilterBG = sc.Subgraph[0];

    if (sc.SetDefaults)
    {
        sc.GraphName = "ATEAM Rotation V3 Chop";
        sc.AutoLoop = 1;
        sc.UpdateAlways = 1;
        sc.GraphRegion = 0;
        ...
        sg_FilterBG.Name = "Filter BG (Speed + Chop)";
        ...
        sc.Input[15].SetString(
            "C:\\Projects\\futures_pipeline\\lab\\output\\rotational-NQ-scale-detection\\");

        // [CHOP-1.0] Choppiness filter
        sc.Input[16].Name = "Enable Choppiness Filter";
        sc.Input[16].SetYesNo(1);  // ON by default for this variant

        sc.Input[17].Name = "Choppiness Study Ref";
        sc.Input[17].SetStudySubgraphValues(0, 0);  // ATEAM_ChoppinessFilter, Subgraph 0 (raw choppiness)

        sc.Input[18].Name = "Choppiness Threshold (block at or above)";
    ...
    if (sc.Input[14].GetYesNo())
    {
        ...
        const int    ChopFilterOn  = sc.Input[16].GetYesNo();
        const float  ChopThresh    = sc.Input[18].GetFloat();
        const int    ChopLookback  = 3;  // fixed at 3 (validated in study)

        RunTestMode(sc, basePath.GetChars(),
                    StepDist, InitialQty, MaxLevels, MaxContractSize,
                    HardStop, MaxFades, sc.TickSize,
                    ChopFilterOn, ChopThresh, ChopLookback);
    ...
    int&    ChopBlockLogged = sc.GetPersistentInt(9);  // 1=already logged CHOP_BLOCKED this watch
    ...
    // [CHOP-2.0] Read choppiness from external ChoppinessFilter study on 250-tick chart.
    // Bar boundaries align with SC's native 250-tick counting (continuous, no midnight reset).
    // Python _aggregate_bars() matches this behavior.
    float ChopVal = 0.0f;
    int   ChopEntryAllowed = 1;  // default: allow
    if (ChopFilterEnabled)
    {
        ...
        ChopVal = (ChopData.GetArraySize() > sc.Index) ? ChopData[sc.Index] : 0.0f;
    ...
    {
        SCString msg;
        msg.Format("Dir:%d Level:%d Pos:%d Anchor:%.2f Price:%.2f Pend:%d Flat:%d FadesL:%d FadesS:%d Speed:%.1f SpeedOff:%d Chop:%.3f ChopOK:%d",
                   Direction, Level, PosQty, AnchorPrice, Price, OrderPending, FlattenPending,
                   FadeCountLong, FadeCountShort, SpeedVal, SpeedFilterOff, ChopVal, ChopEntryAllowed);
    ...
    if (FlattenPending)
    {
        ...
        // [CHOP-1.0] Choppiness gate: skip reversal re-entry if chop >= threshold
        if (ChopFilterEnabled && !ChopEntryAllowed)
        {
            if (CSVEnabled && !ChopBlockLogged)
            {
                WriteCSV(sc, &CSVHeader, "CHOP_BLOCKED",
                    NewDir == 1 ? "LONG" : "SHORT",
                    Price, 0.0, 0, 0, Level, ChopVal,
                    StepDist, MaxLevels, MaxContractSize);
                ChopBlockLogged = 1;
            }
            ResetToWatching();
        ...
        if (Market(NewDir, InitialQty))
        {
            ...
            if (CSVEnabled)
                WriteCSV(sc, &CSVHeader, "REVERSAL_ENTRY",
                    NewDir == 1 ? "LONG" : "SHORT",
                    Price, Price, InitialQty, InitialQty, 0, ChopVal,
                    StepDist, MaxLevels, MaxContractSize);
        }
        return;
    }
    ...
    if (PosQty == 0 && AnchorPrice == 0.0)
    {
        ...
        // [CHOP-1.0] Choppiness gate: skip seed entry if chop >= threshold
        if (ChopFilterEnabled && !ChopEntryAllowed)
        {
            if (CSVEnabled && !ChopBlockLogged)
            {
