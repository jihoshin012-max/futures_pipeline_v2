"""Holdout validation of max daily loss thresholds for rangefade rotation.

Runs the same simulation as max-daily-loss-analysis.py on holdout data
(NQ-250tick-holdout.csv, 12/15/2025 to 3/13/2026) to validate calibration results.

Configs tested:
  1. Baseline: no max daily loss
  2. Max daily loss = $2,000
  3. Max daily loss = $2,500

All configs: LB=100, inner=0.75, outer=1.75, consec stop N=2,
qty=1, no martingale, no step-up, RTH 09:30-15:45, entry at bar close, ddof=0.
...
CALIBRATION = {
    0:    {"total_pnl": 33228, "max_drawdown": 17681, "pnl_dd_ratio": 1.88},
    2000: {"total_pnl": 28535, "max_drawdown": 12583, "pnl_dd_ratio": 2.27},
    2500: {"total_pnl": 33043, "max_drawdown": 15518, "pnl_dd_ratio": 2.13},
}
...
def main():
    ...
    # ── Write holdout CSV ──
    csv_path = OUTPUT_DIR / "holdout-max-loss.csv"
    cols = [
    ...
    summary_path = OUTPUT_DIR / "holdout-max-loss-summary.txt"

    lines = []
    lines.append("=" * 110)
    lines.append("HOLDOUT VALIDATION: MAX DAILY LOSS THRESHOLDS")
    lines.append("=" * 110)
    ...
    lines.append("-" * 110)
    lines.append("")
    comp_hdr = (f"{'Config':>8s}  "
                f"{'Cal PnL':>10s}  {'HO PnL':>10s}  {'Delta':>10s}  "
                f"{'Cal MaxDD':>10s}  {'HO MaxDD':>10s}  {'Delta':>10s}  "
                f"{'Cal Ratio':>9s}  {'HO Ratio':>9s}  {'Delta':>9s}")
    lines.append(comp_hdr)
    lines.append("-" * len(comp_hdr))

    for r in results:
        threshold = int(r["max_daily_loss"])
        cal = CALIBRATION.get(threshold)
        if cal is None:
            continue
        label = f"${threshold}" if threshold > 0 else "OFF"
        dpnl = r["total_pnl"] - cal["total_pnl"]
        ddd = r["max_drawdown"] - cal["max_drawdown"]
        dratio = r["pnl_dd_ratio"] - cal["pnl_dd_ratio"]
        line = (f"{label:>8s}  "
                f"{cal['total_pnl']:>10.2f}  {r['total_pnl']:>10.2f}  {dpnl:>+10.2f}  "
                f"{cal['max_drawdown']:>10.2f}  {r['max_drawdown']:>10.2f}  {ddd:>+10.2f}  "
                f"{cal['pnl_dd_ratio']:>9.3f}  {r['pnl_dd_ratio']:>9.3f}  {dratio:>+9.3f}")
        lines.append(line)
