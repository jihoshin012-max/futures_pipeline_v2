"""Sub-band inner=1.0, outer=3.0 grid: extended targets + switch-back rules.

Base config: LB=100, inner=0.75, outer=1.75, CB N=2, MDL $2500, qty=1,
EMA period=50, EMA threshold=0.1, sub-inner=1.0, sub-outer=3.0.
RTH only (09:30-15:45), entry at bar close, ddof=0.

Sub-band geometry:
  subMid = (innerBand + outerBand) / 2 on failing side
  halfWidth = (outerBand - innerBand) / 2
  sub-inner entry = subMid +/- 1.0 * halfWidth (matches inner/outer bands)
  sub-outer stop  = subMid +/- 3.0 * halfWidth (wide stop)

Target extensions (applied to sub-band trades only):
  1.0x  -- standard (sub-inner band width = 2 * subInnerMult * halfWidth)
  1.5x  -- 50% larger
  2.0x  -- double
  3.0x  -- triple
...
Switch-back rules:
  R0 -- Stay in sub-band mode rest of day
...
SWITCHBACK_RULES = ["R0", "R1", "R2", "R4", "R5"]
...
def main():
    ...
    print(f"\nRunning {total_configs} configs (6 targets x 5 switch-back rules)...")
    ...
    # -- Write grid CSV --
    csv_path = OUTPUT_DIR / "subband-1-3-grid.csv"
    cols = [
    ...
    # -- Write summary --
    BASELINE_PNL_REF = 33043.0
    BASELINE_PNLDD_REF = 2.13

    summary_path = OUTPUT_DIR / "subband-1-3-summary.txt"
    lines = []
    lines.append("=" * 110)
    lines.append("SUB-BAND INNER=1.0, OUTER=3.0: EXTENDED TARGET + SWITCH-BACK RULE GRID")
    lines.append("=" * 110)
    ...
    lines.append("")
    lines.append("Sub-band geometry:")
    lines.append("  subMid = (innerBand + outerBand) / 2 on failing side")
    lines.append("  halfWidth = (outerBand - innerBand) / 2")
    lines.append(f"  sub-inner entry = subMid +/- {SUB_INNER_MULT} * halfWidth")
    lines.append(f"  sub-outer stop  = subMid +/- {SUB_OUTER_MULT} * halfWidth")
    lines.append("")
    lines.append("Target modes:")
    lines.append(f"  1.0x  = standard sub-inner band width (2 * {SUB_INNER_MULT} * halfWidth)")
    lines.append("  1.5x  = 1.5 * standard")
    lines.append("  2.0x  = 2.0 * standard")
    lines.append("  3.0x  = 3.0 * standard")
    ...
    lines.append("")
    lines.append(f"Reference baseline: PnL=${BASELINE_PNL_REF:,.0f}, PnL/DD={BASELINE_PNLDD_REF:.2f}")
    lines.append("")
    ...
    lines.append("")
    lines.append("-" * 110)
    lines.append(f"CONFIGS BEATING REFERENCE BASELINE (PnL>${BASELINE_PNL_REF:,.0f}, PnL/DD>{BASELINE_PNLDD_REF:.2f})")
    lines.append("-" * 110)

    beats_pnl = []
    beats_both = []
    for r in results:
        if r["total_pnl"] > BASELINE_PNL_REF:
            beats_pnl.append(r)
            if r["pnl_dd_ratio"] > BASELINE_PNLDD_REF:
                beats_both.append(r)

    if beats_pnl:
        beats_pnl.sort(key=lambda x: x["total_pnl"], reverse=True)
        lines.append(f"\nConfigs beating baseline PnL (${BASELINE_PNL_REF:,.0f}): {len(beats_pnl)}/{len(results)}")
        for r in beats_pnl:
            dpnl = r["total_pnl"] - BASELINE_PNL_REF
        ...
        lines.append(f"\nNo configs beat baseline PnL (${BASELINE_PNL_REF:,.0f}).")

    if beats_both:
        beats_both.sort(key=lambda x: x["pnl_dd_ratio"], reverse=True)
        lines.append(f"\nConfigs beating BOTH PnL and PnL/DD: {len(beats_both)}/{len(results)}")
        for r in beats_both:
            lines.append(f"  {r['target_mode']:>18s} / {r['switchback']}: "
                         f"PnL=${r['total_pnl']:,.2f}, PnL/DD={r['pnl_dd_ratio']:.3f}, "
                         f"EV/T=${r['ev_per_trade']:.2f}, "
                         f"sub={r['sub_trades']}, sWR={r['sub_win_rate']:.1f}%")
    elif beats_pnl:
        lines.append(f"\nNo configs beat both PnL and PnL/DD thresholds.")
