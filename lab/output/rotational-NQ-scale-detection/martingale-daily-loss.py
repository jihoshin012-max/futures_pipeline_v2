"""Martingale sizing WITH max daily loss limit on rangefade rotation.

Combines the martingale sizing logic from consec-stop-martingale.py
with the max daily loss mechanic from max-daily-loss-analysis.py.

Configs tested:
  1. No martingale, max loss $2,500 (baseline reference)
  2. m2.0 max2, max loss $2,500
  3. m2.0 max3, max loss $2,500
  4. m2.0 max2, NO max loss (comparison)
  5. No martingale, NO max loss (pure baseline)
...
# Configs: (label, mart_mult, mart_max, max_daily_loss)
CONFIGS = [
    ("no-mart_mdl2500",  1.0, 1, 2500.0),
    ("m2.0-max2_mdl2500", 2.0, 2, 2500.0),
    ("m2.0-max3_mdl2500", 2.0, 3, 2500.0),
    ("m2.0-max2_nomdl",  2.0, 2, 0.0),
    ("no-mart_nomdl",    1.0, 1, 0.0),
]
...
def simulate(bars: dict, bands: dict, mart_mult: float, mart_max: int,
    ...
    """
    Run rangefade rotation sim with N=2 circuit breaker, martingale sizing,
    and max daily loss limit.

    mart_mult=1.0, mart_max=1 => no martingale
    ...
    # Consecutive stop state (circuit breaker + martingale)
    consec_long = 0
    consec_short = 0
    last_date = 0
    ...
    def check_daily_halt():
        """Check if max daily loss triggered. Returns True if halted."""
        ...
        if daily_halted:
            if in_position:
                ...
                trades.append({"pnl_dollar": round(pnl_dollar, 2), "qty": trade_qty,
                               "exit_type": "DAILY_HALT", "date": date_int[i], "dir": direction})
    ...
    return {
        ...
        "days_halted_pct": round(days_halted / total_days * 100, 1) if total_days > 0 else 0.0,
    }
...
def main():
    ...
    # -- Run configs --
    results = []
    for label, mult, max_qty, mdl in CONFIGS:
        mdl_str = f"${int(mdl)}" if mdl > 0 else "OFF"
        print(f"  Simulating {label} (mult={mult}, max={max_qty}, maxDailyLoss={mdl_str})...")
        res = simulate(bars, bands, mult, max_qty, mdl)
        res["config"] = label
        res["mult"] = mult
        res["max_qty"] = max_qty
        res["max_daily_loss"] = mdl
        results.append(res)

    # -- Write CSV --
    csv_path = OUTPUT_DIR / "martingale-daily-loss.csv"
    cols = [
        "config", "mult", "max_qty", "max_daily_loss",
        ...
        "largest_single_loss", "days_halted", "days_halted_pct",
    ]
    ...
    # -- Write summary --
    summary_path = OUTPUT_DIR / "martingale-daily-loss-summary.txt"

    lines = []
    lines.append("=" * 120)
    lines.append("MARTINGALE + MAX DAILY LOSS TEST -- RANGEFADE ROTATION (N=2 CIRCUIT BREAKER)")
    lines.append("=" * 120)
    ...
    lines.append("  - Win resets counter to 0, loss increments")
    lines.append("  - Daily reset, forced-opposite resets both counters")
    lines.append("  - With N=2, counter can only be 0 or 1 when entering")
    lines.append("")
    lines.append("Max daily loss logic:")
    lines.append("  - Track cumulative PnL for current day (including costs)")
    lines.append("  - When daily PnL <= -maxDailyLoss, flatten position and stop for day")
    lines.append("  - Resets at start of each new trading day")
    ...
    hdr = (f"{'Config':>20s}  {'Trades':>6s}  {'PnL($)':>10s}  {'Up':>3s}  {'Dn':>3s}  "
           ...
           f"{'MaxLoss':>9s}  {'Halts':>5s}  {'Hlt%':>5s}")
    lines.append(hdr)
    lines.append("-" * len(hdr))

    for r in results:
        line = (f"{r['config']:>20s}  {r['total_trades']:>6d}  {r['total_pnl']:>10.2f}  "
                ...
                f"{r['largest_single_loss']:>9.2f}  {r['days_halted']:>5d}  {r['days_halted_pct']:>5.1f}")
        lines.append(line)
    ...
    # Pure baseline (config 5)
    pure_baseline = results[4]  # no-mart_nomdl
    lines.append(f"Pure baseline (no martingale, no max loss): {pure_baseline['total_trades']} trades, "
                 f"PnL=${pure_baseline['total_pnl']:.2f}, MaxDD=${pure_baseline['max_drawdown']:.2f}, "
                 f"PnL/DD={pure_baseline['pnl_dd_ratio']:.3f}, Worst=${pure_baseline['worst_day']:.2f}")
    lines.append("")

    # Deltas vs pure baseline
    lines.append("Deltas vs pure baseline (no-mart_nomdl):")
    for r in results:
        if r["config"] == "no-mart_nomdl":
            continue
        dpnl = r["total_pnl"] - pure_baseline["total_pnl"]
        ddd = r["max_drawdown"] - pure_baseline["max_drawdown"]
        dratio = r["pnl_dd_ratio"] - pure_baseline["pnl_dd_ratio"]
        dworst = r["worst_day"] - pure_baseline["worst_day"]
        lines.append(f"  {r['config']:>20s}: PnL {dpnl:+10.2f}, MaxDD {ddd:+9.2f}, "
                     f"PnL/DD {dratio:+7.3f}, Worst {dworst:+9.2f}, "
                     f"MaxSingleLoss {r['largest_single_loss']:.2f}, "
                     f"Halts {r['days_halted']:>3d} ({r['days_halted_pct']:.1f}%)")
    lines.append("")

    # Martingale effect WITH max daily loss (config 2 vs config 1)
    mdl_baseline = results[0]  # no-mart_mdl2500
    lines.append("Effect of adding martingale WITH $2,500 max daily loss (vs no-mart_mdl2500):")
    for r in [results[1], results[2]]:  # m2.0-max2_mdl2500, m2.0-max3_mdl2500
        dpnl = r["total_pnl"] - mdl_baseline["total_pnl"]
        ddd = r["max_drawdown"] - mdl_baseline["max_drawdown"]
        dratio = r["pnl_dd_ratio"] - mdl_baseline["pnl_dd_ratio"]
        lines.append(f"  {r['config']:>20s}: PnL {dpnl:+10.2f}, MaxDD {ddd:+9.2f}, "
                     f"PnL/DD {dratio:+7.3f}, Worst {r['worst_day']:.2f}, "
                     f"MaxSingleLoss {r['largest_single_loss']:.2f}, "
                     f"Halts {r['days_halted']:>3d} ({r['days_halted_pct']:.1f}%)")
    lines.append("")

    # Max daily loss effect on martingale (config 2 vs config 4)
    lines.append("Effect of adding $2,500 max daily loss TO martingale (m2.0-max2):")
    with_mdl = results[1]   # m2.0-max2_mdl2500
    without_mdl = results[3]  # m2.0-max2_nomdl
    dpnl = with_mdl["total_pnl"] - without_mdl["total_pnl"]
    ddd = with_mdl["max_drawdown"] - without_mdl["max_drawdown"]
    dratio = with_mdl["pnl_dd_ratio"] - without_mdl["pnl_dd_ratio"]
    dworst = with_mdl["worst_day"] - without_mdl["worst_day"]
    lines.append(f"  PnL delta:       {dpnl:+10.2f}")
    lines.append(f"  MaxDD delta:     {ddd:+9.2f}")
    lines.append(f"  PnL/DD delta:    {dratio:+7.3f}")
    lines.append(f"  Worst day delta: {dworst:+9.2f}")
    lines.append(f"  Halts:           {with_mdl['days_halted']} ({with_mdl['days_halted_pct']:.1f}%)")
    lines.append("")

    lines.append("NOTE: With N=2 circuit breaker, per-side counter can only be 0 or 1 when entering.")
